...
class ImageCaptionModel(nn.Module):
    def __init__(self, config: dict):
        ...
        self.input_layer = nn.Sequential(
            nn.Dropout(0.25), 
            nn.Conv1d(2048, self.nn_map_size, 1),
...
class RNNOneLayerSimplified(nn.Module):
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        #num_rnn_layers = initial_hidden_state.size()[0]
...
class RNN(nn.Module):
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        #maxpooling
        processed_cnn_features_pooled = self.pooling(processed_cnn_features)
        processed_cnn_features_pooled = torch.squeeze(processed_cnn_features_pooled)

        processed_cnn_features = torch.transpose(processed_cnn_features, 1, 2)
        ...
        for i in range(sequence_length):
            ...
            for j in range(self.num_rnn_layers):
                if j == 0:
                    input_for_the_first_layer = torch.cat((input_tokens, processed_cnn_features_pooled), dim=1)
                    hidden_states_each_layer[j] = self.cells[j].forward(input_for_the_first_layer, hidden_states_each_layer[j])

                    attention_mechanism = self.attention(hidden_states_each_layer[j])
                    attention_mechanism = torch.unsqueeze(attention_mechanism, dim=2)

                    input_for_the_first_layer = processed_cnn_features * attention_mechanism
                    input_for_the_first_layer = torch.sum(input_for_the_first_layer, dim = 1)

                else:
                    hidden_states_each_layer[j] = self.cells[j].forward(input_for_the_first_layer, hidden_states_each_layer[j-1])
                    input_for_the_first_layer = hidden_states_each_layer[j][:,:self.hidden_state_size]
            ...
            #logits_i = output_layer(hidden_states_each_layer[-1])
            logits_sequence.append(logits_i)
            predictions = torch.argmax(logits_i, dim=1)
            if i < sequence_length - 1:
                if is_train:
