    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        for i in range(sequence_length):
            ...
            for j in range(self.num_rnn_layers):
                current_hidden_state[j] = self.cells[j].forward(input_for_the_first_layer, current_hidden_state[j])
                current_hidden_state[j] = torch.unsqueeze(current_hidden_state[j], dim = 0)
